...
public class XMLMatcher extends Matcher {
  ...
  public static Charset detectBomCharset(InputStream in) throws IOException {
    byte[] bom = new byte[4];
    int bytesRead = in.read(bom, 0, 4);
  ...
  /**
   * Create a {@link java.io.Reader} instance that will obey potential ByteOrderMark BOMs.
   ...
   * @return a {@link java.io.Reader} instance respecting the encoding if defined by a ByteOrderMark BOM
   */
  private InputStreamReader createReader(final SeekableInputStream sis) throws IOException {
    Charset charset = detectBomCharset(sis);
    sis.seek(0);
    if (charset == null) {
